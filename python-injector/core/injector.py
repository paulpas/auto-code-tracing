...
def process_service_files(self, service_dir: pathlib.Path,
                          file_extensions: List[str]) -> int:
    """High‑level driver – returns the number of files successfully injected."""
    # ✅ CALL the helper – now we get a dict, not a function object
    files_by_lang = self._organize_files_by_language(service_dir,
                                                     file_extensions)

    if not files_by_lang:
        log.info("🔍 No source files found in %s", service_dir)
        return 0

    total_processed = 0
    for language, files in files_by_lang.items():
        try:
            sorted_files = self._sort_files_for_language(files)
            processed = self._process_language_group(language,
                                                     sorted_files,
                                                     service_dir)
            total_processed += processed
        except Exception as exc:
            log.error("❌ Error in language group processing: %s", exc)
...
def _organize_files_by_language(self,
                                service_dir: pathlib.Path,
                                extensions: List[str]) -> Dict[str, List[pathlib.Path]]:
    """
    Scan *service_dir* and return a mapping:
        { language_name : [Path, …] }
    """
    files_by_lang: defaultdict[str, List[pathlib.Path]] = defaultdict(list)

    for path in service_dir.rglob("*"):
        if path.suffix in extensions:
            lang = language_for_extension.get(path.suffix)
            if lang:
                files_by_lang[lang].append(path)

    # Cast to a plain dict so callers can safely use .items()
